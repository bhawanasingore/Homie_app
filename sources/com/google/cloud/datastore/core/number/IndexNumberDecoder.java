...
public class IndexNumberDecoder {
    ...
    private void updateResultDoubleState() {
        if (this.doubleResultRepProblemMessage == null) {
            ...
            if (i == Integer.MAX_VALUE) {
                ...
                this.resultAsDouble = Double.longBitsToDouble(doubleValueAsLong | j);
            }
        }
    }
    ...
    public int decode(boolean descending, byte[] buffer, int offset) {
        ...
        int inverter2 = invertEncoding ? (byte) 255 : 0;
        int b2 = b ^ inverter2;
        ...
        if (marker == -4) {
            ...
            significand2 = 0 | (((long) (b2 & ((~(-1 << (bufferPos + 1))) & 126))) << (writeBit - 1));
        } else if (marker == 1) {
            inverter = inverter2;
