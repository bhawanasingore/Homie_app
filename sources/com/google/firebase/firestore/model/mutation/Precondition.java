...
public final class Precondition {
    ...
    public boolean isValidFor(MaybeDocument maybeDoc) {
        if (this.updateTime == null) {
            Boolean bool = this.exists;
            if (bool == null) {
                Assert.hardAssert(isNone(), "Precondition should be empty", new Object[0]);
                return true;
            } else if (bool.booleanValue() == (maybeDoc instanceof Document)) {
                return true;
            } else {
                return false;
            }
        } else if (!(maybeDoc instanceof Document) || !maybeDoc.getVersion().equals(this.updateTime)) {
            return false;
        } else {
            return true;
        }
