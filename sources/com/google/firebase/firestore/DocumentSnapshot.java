...
public class DocumentSnapshot {
    ...
    static DocumentSnapshot fromNoDocument(FirebaseFirestore firestore2, DocumentKey key2, boolean fromCache, boolean hasPendingWrites) {
        return new DocumentSnapshot(firestore2, key2, (Document) null, fromCache, hasPendingWrites);
    }
    ...
    public Map<String, Object> getData(ServerTimestampBehavior serverTimestampBehavior) {
        ...
        UserDataWriter userDataWriter = new UserDataWriter(firebaseFirestore, firebaseFirestore.getFirestoreSettings().areTimestampsInSnapshotsEnabled(), serverTimestampBehavior);
    ...
    public Object get(FieldPath fieldPath, ServerTimestampBehavior serverTimestampBehavior) {
        Preconditions.checkNotNull(fieldPath, "Provided field path must not be null.");
        Preconditions.checkNotNull(serverTimestampBehavior, "Provided serverTimestampBehavior value must not be null.");
        return getInternal(fieldPath.getInternalPath(), serverTimestampBehavior, this.firestore.getFirestoreSettings().areTimestampsInSnapshotsEnabled());
    }
    ...
    public Date getDate(String field, ServerTimestampBehavior serverTimestampBehavior) {
        ...
        Preconditions.checkNotNull(serverTimestampBehavior, "Provided serverTimestampBehavior value must not be null.");
        return (Date) castTypedValue(getInternal(FieldPath.fromDotSeparatedPath(field).getInternalPath(), serverTimestampBehavior, false), field, Date.class);
    }
    ...
    public Timestamp getTimestamp(String field, ServerTimestampBehavior serverTimestampBehavior) {
        Preconditions.checkNotNull(field, "Provided field path must not be null.");
        Preconditions.checkNotNull(serverTimestampBehavior, "Provided serverTimestampBehavior value must not be null.");
        return (Timestamp) castTypedValue(getInternal(FieldPath.fromDotSeparatedPath(field).getInternalPath(), serverTimestampBehavior, true), field, Timestamp.class);
    }
    ...
    private Object getInternal(FieldPath fieldPath, ServerTimestampBehavior serverTimestampBehavior, boolean timestampsInSnapshots) {
        Value val;
        Document document = this.doc;
        if (document == null || (val = document.getField(fieldPath)) == null) {
            return null;
        }
        return new UserDataWriter(this.firestore, timestampsInSnapshots, serverTimestampBehavior).convertValue(val);
    }
    ...
    public int hashCode() {
        ...
        Document document = this.doc;
        return ((hash + (document != null ? document.hashCode() : 0)) * 31) + this.metadata.hashCode();
    }
