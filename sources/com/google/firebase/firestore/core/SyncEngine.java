...
public class SyncEngine implements RemoteStore.RemoteStoreCallback {
    ...
    public void handleRejectedListen(int targetId, Status error) {
        ...
        if (limboKey != null) {
            ...
            handleRemoteEvent(new RemoteEvent(SnapshotVersion.NONE, Collections.emptyMap(), Collections.emptySet(), Collections.singletonMap(limboKey, new NoDocument(limboKey, SnapshotVersion.NONE, false)), Collections.singleton(limboKey)));
            return;
        }
        this.localStore.releaseTarget(targetId);
    ...
    private void emitNewSnapsAndNotifyLocalStore(ImmutableSortedMap<DocumentKey, MaybeDocument> changes, RemoteEvent remoteEvent) {
    ...
    private void updateTrackedLimboDocuments(List<LimboDocumentChange> limboChanges, int targetId) {
        for (LimboDocumentChange limboChange : limboChanges) {
            int i = AnonymousClass1.$SwitchMap$com$google$firebase$firestore$core$LimboDocumentChange$Type[limboChange.getType().ordinal()];
            if (i == 1) {
    ...
    private void pumpEnqueuedLimboResolutions() {
        while (!this.enqueuedLimboResolutions.isEmpty() && this.activeLimboTargetsByKey.size() < this.maxConcurrentLimboResolutions) {
            DocumentKey key = this.enqueuedLimboResolutions.remove();
