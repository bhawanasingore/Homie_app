...
public final class Query {
    ...
    public Filter.Operator findFilterOperator(List<Filter.Operator> operators) {
        for (Filter filter : this.filters) {
            if (filter instanceof FieldFilter) {
                Filter.Operator filterOp = ((FieldFilter) filter).getOperator();
    ...
    private boolean matchesBounds(Document doc) {
        ...
        if (bound2 == null || !bound2.sortsBeforeDocument(getOrderBy(), doc)) {
            return true;
        }
        return false;
    }
    ...
    public boolean matches(Document doc) {
        return matchesPathAndCollectionGroup(doc) && matchesOrderBy(doc) && matchesFilters(doc) && matchesBounds(doc);
    }
    ...
    public Target toTarget() {
        ...
        if (this.memoizedTarget == null) {
            if (this.limitType == LimitType.LIMIT_TO_FIRST) {
                ...
                Bound newStartAt = bound != null ? new Bound(bound.getPosition(), !this.endAt.isBefore()) : null;
                Bound bound2 = this.startAt;
                if (bound2 != null) {
                    newEndAt = new Bound(bound2.getPosition(), !this.startAt.isBefore());
                } else {
                    newEndAt = null;
                }
