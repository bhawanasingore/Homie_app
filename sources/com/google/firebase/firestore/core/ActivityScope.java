...
public class ActivityScope {
    ...
    private static void onActivityStopCallOnce(Activity activity, Runnable callback) {
        ...
        activity.runOnUiThread(ActivityScope$$Lambda$1.lambdaFactory$(activity, callback));
    }
    ...
    private static void onFragmentActivityStopCallOnce(FragmentActivity activity, Runnable callback) {
        activity.runOnUiThread(ActivityScope$$Lambda$2.lambdaFactory$(activity, callback));
    }
    ...
    public static ListenerRegistration bind(Activity activity, ListenerRegistration registration) {
        if (activity != null) {
            if (activity instanceof FragmentActivity) {
                ...
                onFragmentActivityStopCallOnce((FragmentActivity) activity, ActivityScope$$Lambda$3.lambdaFactory$(registration));
            } else {
                registration.getClass();
                onActivityStopCallOnce(activity, ActivityScope$$Lambda$4.lambdaFactory$(registration));
            }
        }
        return registration;
    }
