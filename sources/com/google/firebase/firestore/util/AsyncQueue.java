...
public class AsyncQueue {
    ...
    public static <TResult> Task<TResult> callTask(Executor executor2, Callable<Task<TResult>> task) {
        ...
        executor2.execute(AsyncQueue$$Lambda$1.lambdaFactory$(task, executor2, tcs));
        return tcs.getTask();
    }

    static /* synthetic */ void lambda$callTask$1(Callable task, Executor executor2, TaskCompletionSource tcs) {
        try {
            ((Task) task.call()).continueWith(executor2, AsyncQueue$$Lambda$7.lambdaFactory$(tcs));
        } catch (Exception e) {
            tcs.setException(e);
        } catch (Throwable t) {
    ...
    private class SynchronizedShutdownAwareExecutor implements Executor {
        ...
        SynchronizedShutdownAwareExecutor() {
            ...
            AnonymousClass1 r1 = new ScheduledThreadPoolExecutor(1, threadFactory, AsyncQueue.this) {
        ...
        public <T> Task<T> executeAndReportResult(Callable<T> task) {
            ...
            try {
                execute(AsyncQueue$SynchronizedShutdownAwareExecutor$$Lambda$2.lambdaFactory$(completionSource, task));
            } catch (RejectedExecutionException e) {
                Logger.warn(AsyncQueue.class.getSimpleName(), "Refused to enqueue task after panic", new Object[0]);
            }
        ...
        public synchronized Task<Void> executeAndInitiateShutdown(Runnable task) {
            ...
            Task<Void> t = executeAndReportResult(AsyncQueue$SynchronizedShutdownAwareExecutor$$Lambda$3.lambdaFactory$(task));
    ...
    public Task<Void> enqueue(Runnable task) {
        return enqueue(AsyncQueue$$Lambda$2.lambdaFactory$(task));
    }
    ...
    public void panic(Throwable t) {
        ...
        new Handler(Looper.getMainLooper()).post(AsyncQueue$$Lambda$3.lambdaFactory$(t));
    }

    static /* synthetic */ void lambda$panic$3(Throwable t) {
        if (t instanceof OutOfMemoryError) {
            OutOfMemoryError error = new OutOfMemoryError("Firestore (21.4.3) ran out of memory. Check your queries to make sure they are not loading an excessive amount of data.");
            error.initCause(t);
            throw error;
        }
        throw new RuntimeException("Internal error in Cloud Firestore (21.4.3).", t);
    }

    public void runSync(Runnable task) throws InterruptedException {
        Semaphore done = new Semaphore(0);
        Throwable[] t = new Throwable[1];
        enqueueAndForget(AsyncQueue$$Lambda$4.lambdaFactory$(task, t, done));
        done.acquire(1);
        if (t[0] != null) {
    ...
    public void runDelayedTasksUntil(TimerId lastTimerId) throws InterruptedException {
        runSync(AsyncQueue$$Lambda$5.lambdaFactory$(this, lastTimerId));
    }

    static /* synthetic */ void lambda$runDelayedTasksUntil$6(AsyncQueue asyncQueue, TimerId lastTimerId) {
        Assert.hardAssert(lastTimerId == TimerId.ALL || asyncQueue.containsDelayedTask(lastTimerId), "Attempted to run tasks until missing TimerId: %s", lastTimerId);
