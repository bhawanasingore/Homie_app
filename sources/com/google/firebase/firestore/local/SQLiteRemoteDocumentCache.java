...
final class SQLiteRemoteDocumentCache implements RemoteDocumentCache {
    ...
    public void add(MaybeDocument maybeDocument, SnapshotVersion readTime) {
        ...
        this.db.execute("INSERT OR REPLACE INTO remote_documents (path, read_time_seconds, read_time_nanos, contents) VALUES (?, ?, ?, ?)", path, Long.valueOf(timestamp.getSeconds()), Integer.valueOf(timestamp.getNanoseconds()), message.toByteArray());
    ...
    public ImmutableSortedMap<DocumentKey, Document> getAllDocumentsMatchingQuery(Query query, SnapshotVersion sinceReadTime) {
        ...
        if (sinceReadTime.equals(SnapshotVersion.NONE)) {
            sqlQuery = this.db.query("SELECT path, contents FROM remote_documents WHERE path >= ? AND path < ?").binding(prefixPath, prefixSuccessorPath);
        } else {
            sqlQuery = this.db.query("SELECT path, contents FROM remote_documents WHERE path >= ? AND path < ?AND (read_time_seconds > ? OR (read_time_seconds = ? AND read_time_nanos > ?))").binding(prefixPath, prefixSuccessorPath, Long.valueOf(readTime.getSeconds()), Long.valueOf(readTime.getSeconds()), Integer.valueOf(readTime.getNanoseconds()));
        }
