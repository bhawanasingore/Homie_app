...
final class SQLiteTargetCache implements TargetCache {
    ...
    public void start() {
        boolean z = true;
        if (this.db.query("SELECT highest_target_id, highest_listen_sequence_number, last_remote_snapshot_version_seconds, last_remote_snapshot_version_nanos, target_count FROM target_globals LIMIT 1").first(SQLiteTargetCache$$Lambda$1.lambdaFactory$(this)) != 1) {
            z = false;
        }
    ...
    public void forEachTarget(Consumer<TargetData> consumer) {
        this.db.query("SELECT target_proto FROM targets").forEach(SQLiteTargetCache$$Lambda$2.lambdaFactory$(this, consumer));
    }
    ...
    public int removeQueries(long upperBound, SparseArray<?> activeTargetIds) {
        int[] count = new int[1];
        this.db.query("SELECT target_id FROM targets WHERE last_listen_sequence_number <= ?").binding(Long.valueOf(upperBound)).forEach(SQLiteTargetCache$$Lambda$3.lambdaFactory$(this, activeTargetIds, count));
        writeMetadata();
        return count[0];
    }
    ...
    public TargetData getTargetData(com.google.firebase.firestore.core.Target target) {
        ...
        this.db.query("SELECT target_proto FROM targets WHERE canonical_id = ?").binding(canonicalId).forEach(SQLiteTargetCache$$Lambda$4.lambdaFactory$(this, target, result));
    ...
    public ImmutableSortedSet<DocumentKey> getMatchingKeysForTargetId(int targetId) {
        ...
        this.db.query("SELECT path FROM target_documents WHERE target_id = ?").binding(Integer.valueOf(targetId)).forEach(SQLiteTargetCache$$Lambda$5.lambdaFactory$(holder));
        return holder.keys;
    }
