...
final class SQLiteMutationQueue implements MutationQueue {
    ...
    private void loadNextBatchIdAcrossAllUsers() {
        ...
        for (String uid2 : uids) {
            this.db.query("SELECT MAX(batch_id) FROM mutations WHERE uid = ?").binding(uid2).forEach(SQLiteMutationQueue$$Lambda$3.lambdaFactory$(this));
        }
        this.nextBatchId++;
    }
    ...
    public MutationBatch lookupMutationBatch(int batchId) {
        return (MutationBatch) this.db.query("SELECT SUBSTR(mutations, 1, ?) FROM mutations WHERE uid = ? AND batch_id = ?").binding(Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid, Integer.valueOf(batchId)).firstValue(SQLiteMutationQueue$$Lambda$4.lambdaFactory$(this, batchId));
    }

    public MutationBatch getNextMutationBatchAfterBatchId(int batchId) {
        return (MutationBatch) this.db.query("SELECT batch_id, SUBSTR(mutations, 1, ?) FROM mutations WHERE uid = ? AND batch_id >= ? ORDER BY batch_id ASC LIMIT 1").binding(Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid, Integer.valueOf(batchId + 1)).firstValue(SQLiteMutationQueue$$Lambda$5.lambdaFactory$(this));
    }

    public int getHighestUnacknowledgedBatchId() {
        return ((Integer) this.db.query("SELECT IFNULL(MAX(batch_id), ?) FROM mutations WHERE uid = ?").binding(-1, this.uid).firstValue(SQLiteMutationQueue$$Lambda$6.lambdaFactory$())).intValue();
    }

    public List<MutationBatch> getAllMutationBatches() {
        List<MutationBatch> result = new ArrayList<>();
        this.db.query("SELECT batch_id, SUBSTR(mutations, 1, ?) FROM mutations WHERE uid = ? ORDER BY batch_id ASC").binding(Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid).forEach(SQLiteMutationQueue$$Lambda$7.lambdaFactory$(this, result));
        return result;
    }
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingDocumentKey(DocumentKey documentKey) {
        ...
        List<MutationBatch> result = new ArrayList<>();
        this.db.query("SELECT m.batch_id, SUBSTR(m.mutations, 1, ?) FROM document_mutations dm, mutations m WHERE dm.uid = ? AND dm.path = ? AND dm.uid = m.uid AND dm.batch_id = m.batch_id ORDER BY dm.batch_id").binding(Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid, path).forEach(SQLiteMutationQueue$$Lambda$8.lambdaFactory$(this, result));
        return result;
    }
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingDocumentKeys(Iterable<DocumentKey> documentKeys) {
        ...
        SQLitePersistence.LongQuery longQuery = new SQLitePersistence.LongQuery(this.db, "SELECT DISTINCT dm.batch_id, SUBSTR(m.mutations, 1, ?) FROM document_mutations dm, mutations m WHERE dm.uid = ? AND dm.path IN (", Arrays.asList(new Object[]{Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid}), args, ") AND dm.uid = m.uid AND dm.batch_id = m.batch_id ORDER BY dm.batch_id");
        List<MutationBatch> result = new ArrayList<>();
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingQuery(Query query) {
        ...
        List<MutationBatch> result = new ArrayList<>();
        this.db.query("SELECT dm.batch_id, dm.path, SUBSTR(m.mutations, 1, ?) FROM document_mutations dm, mutations m WHERE dm.uid = ? AND dm.path >= ? AND dm.path < ? AND dm.uid = m.uid AND dm.batch_id = m.batch_id ORDER BY dm.batch_id").binding(Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid, prefixPath, prefixSuccessorPath).forEach(SQLiteMutationQueue$$Lambda$11.lambdaFactory$(this, result, prefix.length() + 1));
        return result;
    }
    ...
    public MutationBatch decodeInlineMutationBatch(int batchId, byte[] bytes) {
        try {
            ...
            while (accumulator.more) {
                this.db.query("SELECT SUBSTR(mutations, ?, ?) FROM mutations WHERE uid = ? AND batch_id = ?").binding(Integer.valueOf((accumulator.numChunks() * BLOB_MAX_INLINE_LENGTH) + 1), Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid, Integer.valueOf(batchId)).first(accumulator);
            }
