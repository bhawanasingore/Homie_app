...
class SQLiteLruReferenceDelegate implements ReferenceDelegate, LruDelegate {
    ...
    public int removeOrphanedDocuments(long upperBound) {
        ...
        while (true) {
            ...
            if (this.persistence.query("select path from target_documents group by path having COUNT(*) = 1 AND target_id = 0 AND sequence_number <= ? LIMIT ?").binding(Long.valueOf(upperBound), 100).forEach(SQLiteLruReferenceDelegate$$Lambda$3.lambdaFactory$(this, count)) == 100) {
                z = true;
            }
            resultsRemaining = z;
        }
    ...
    static /* synthetic */ void lambda$removeOrphanedDocuments$2(SQLiteLruReferenceDelegate sQLiteLruReferenceDelegate, int[] count, Cursor row) {
        DocumentKey key = DocumentKey.fromPath(EncodedPath.decodeResourcePath(row.getString(0)));
        if (!sQLiteLruReferenceDelegate.isPinned(key)) {
            count[0] = count[0] + 1;
            sQLiteLruReferenceDelegate.persistence.getRemoteDocumentCache().remove(key);
            sQLiteLruReferenceDelegate.removeSentinel(key);
        }
    }
