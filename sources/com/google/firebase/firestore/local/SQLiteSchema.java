...
class SQLiteSchema {
    ...
    static /* synthetic */ void lambda$removeAcknowledgedMutations$2(SQLiteSchema sQLiteSchema, Cursor mutationQueueEntry) {
        ...
        new SQLitePersistence.Query(sQLiteSchema.db, "SELECT batch_id FROM mutations WHERE uid = ? AND batch_id <= ?").binding(uid, Long.valueOf(lastAcknowledgedBatchId)).forEach(SQLiteSchema$$Lambda$14.lambdaFactory$(sQLiteSchema, uid));
    }
    ...
    static /* synthetic */ void lambda$dropLastLimboFreeSnapshotVersion$6(SQLiteSchema sQLiteSchema, Cursor cursor) {
        ...
        try {
            ...
            sQLiteSchema.db.execSQL("UPDATE targets SET target_proto = ? WHERE target_id = ?", new Object[]{targetProto.toByteArray(), Integer.valueOf(targetId)});
    ...
    private void ensureSequenceNumbers() {
        Long boxedSequenceNumber = (Long) new SQLitePersistence.Query(this.db, "SELECT highest_listen_sequence_number FROM target_globals LIMIT 1").firstValue(SQLiteSchema$$Lambda$7.lambdaFactory$());
    ...
    private void createV8CollectionParentsIndex() {
        ...
        Consumer<ResourcePath> addEntry = SQLiteSchema$$Lambda$10.lambdaFactory$(new MemoryIndexManager.MemoryCollectionParentIndex(), this.db.compileStatement("INSERT OR REPLACE INTO collection_parents (collection_id, parent) VALUES (?, ?)"));
        new SQLitePersistence.Query(this.db, "SELECT path FROM remote_documents").forEach(SQLiteSchema$$Lambda$11.lambdaFactory$(addEntry));
        new SQLitePersistence.Query(this.db, "SELECT path FROM document_mutations").forEach(SQLiteSchema$$Lambda$12.lambdaFactory$(addEntry));
    }
    ...
    private void rewriteCanonicalIds() {
        new SQLitePersistence.Query(this.db, "SELECT target_id, target_proto FROM targets").forEach(SQLiteSchema$$Lambda$13.lambdaFactory$(this));
    }
    ...
    static /* synthetic */ void lambda$rewriteCanonicalIds$13(SQLiteSchema sQLiteSchema, Cursor cursor) {
        ...
        try {
            ...
            sQLiteSchema.db.execSQL("UPDATE targets SET canonical_id  = ? WHERE target_id = ?", new Object[]{updatedCanonicalId, Integer.valueOf(targetId)});
        } catch (InvalidProtocolBufferException e) {
            throw Assert.fail("Failed to decode Query data for target %s", Integer.valueOf(targetId));
        }
    }
