...
public class DocumentReference {
    ...
    static /* synthetic */ DocumentSnapshot lambda$get$0(DocumentReference documentReference, Task task) throws Exception {
        ...
        return new DocumentSnapshot(documentReference.firestore, documentReference.key, doc, true, doc != null && doc.hasLocalMutations());
    }
    ...
    private Task<DocumentSnapshot> getViaSnapshotListener(Source source) {
        ...
        registration.setResult(addSnapshotListenerInternal(Executors.DIRECT_EXECUTOR, options, (Activity) null, DocumentReference$$Lambda$2.lambdaFactory$(res, registration, source)));
        return res.getTask();
    }
    ...
    private ListenerRegistration addSnapshotListenerInternal(Executor userExecutor, EventManager.ListenOptions options, Activity activity, EventListener<DocumentSnapshot> userListener) {
        AsyncEventListener<ViewSnapshot> asyncListener = new AsyncEventListener<>(userExecutor, DocumentReference$$Lambda$3.lambdaFactory$(this, userListener));
    ...
    static /* synthetic */ void lambda$addSnapshotListenerInternal$2(DocumentReference documentReference, EventListener userListener, ViewSnapshot snapshot, FirebaseFirestoreException error) {
        ...
        Document document = snapshot.getDocuments().getDocument(documentReference.key);
        if (document != null) {
            documentSnapshot = DocumentSnapshot.fromDocument(documentReference.firestore, document, snapshot.isFromCache(), snapshot.getMutatedKeys().contains(document.getKey()));
        } else {
            documentSnapshot = DocumentSnapshot.fromNoDocument(documentReference.firestore, documentReference.key, snapshot.isFromCache(), false);
        }
